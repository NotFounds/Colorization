...
class Colorization(chainer.Chain):
    ...
    def __call__(self, x):
        ...
        x5 = F.dropout(self.bn5(self.conv5(F.relu(x4))), ratio=0.7)
        x6 = F.dropout(self.bn6(self.conv6(F.relu(x5))), ratio=0.7)
        x7 = F.dropout(self.bn7(self.conv7(F.relu(x6))), ratio=0.7)
        x8 = F.dropout(self.bn8(self.conv8(F.relu(x7))), ratio=0.7)

        y1 = F.concat((F.dropout(self.dbn1(self.dconv1(F.relu(x8))), ratio=.7), x7))
...
class Evalution(chainer.Chain):
    ...
    def __call__(self, x, t=None):
        ...
        if t is None:
            return self.y
        self.loss = hsv_mean_squared_error(self.y, t)
        self.accuracy = F.r2_score(self.y, t)
        chainer.report({'loss': self.loss, 'accuracy': self.accuracy}, self)
        return self.loss

def hsv_mean_squared_error(x0, x1):
    loss_s = F.mean_squared_error(x0[1], x1[1])
    diff  = F.absolute(x0[0] - x1[0])
    diff_ = 1 - diff
    diff_min = F.minimum(diff, diff_)
    sqrt = F.square(diff_min)
    mse = F.sum(sqrt) / 65535.0
    loss_h = F.sqrt(mse)
    return (loss_s + loss_h) / 2.0