...
class Colorization(chainer.Chain):
    ...
    def __call__(self, x, test=False, dropout=True):
        x1 = self.conv1(x)
        x2 = self.bn2(self.conv2(F.relu(x1)), test=test)
        x3 = self.bn3(self.conv3(F.relu(x2)), test=test)
        x4 = self.bn4(self.conv4(F.relu(x3)), test=test)
        x5 = self.bn5(self.conv5(F.relu(x4)), test=test)
        x6 = self.bn6(self.conv6(F.relu(x5)), test=test)
        x7 = self.bn7(self.conv7(F.relu(x6)), test=test)
        x8 = self.bn8(self.conv8(F.relu(x7)), test=test)        

        y1 = F.concat((F.dropout(self.dbn1(self.dconv1(F.relu(x8)), test=test), train=dropout), x7))
        y2 = F.concat((F.dropout(self.dbn2(self.dconv2(F.relu(y1)), test=test), train=dropout), x6))
        y3 = F.concat((F.dropout(self.dbn3(self.dconv3(F.relu(y2)), test=test), train=dropout), x5))
        y4 = F.concat((self.dbn4(self.dconv4(F.relu(y3)), test=test), x4))
        y5 = F.concat((self.dbn5(self.dconv5(F.relu(y4)), test=test), x3))
        y6 = F.concat((self.dbn6(self.dconv6(F.relu(y5)), test=test), x2))
        y7 = F.concat((self.dbn7(self.dconv7(F.relu(y6)), test=test), x1))
